...
// export class MazecetricAutomata implements GenerationAlgorithm {
// 	get name(): string { return "Mazecetric Automata" } 


//     constructor() { }
//     private mazecetric(rows: number, cols: number, cells: number, generations: number): number[][] {
...
//             matrix = mazecetricAutomataGetNextGeneration(matrix);
//         }
...
//         const tiles: Tile[][] = this.mazecetric(dimensions.rows, dimensions.cols, Math.ceil(dimensions.rows * dimensions.cols / 4), 50).map(row => row.map(val => {
//             switch (val) {
...
// export function mazecetricAutomataGetNextGeneration(matrix: number[][]): number[][] {
//     if(matrix.length === 0) {
//         return [];
//     }
...
//             if (neighbors >= 1 && neighbors <= 4 && matrix[currentRow][currentCol] === 1) {
