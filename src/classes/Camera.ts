...
export class Camera implements ICamera {
    ...
    constructor(map: GameMap, position: Vector2, direction: Vector2) {
        this.map = map;
    ...
    setDirection(direction: Vector2): Camera {
        return new Camera(this.map, this.position, direction);
    }

    setPosition(position: Vector2): Camera {
        return new Camera(this.map, position, this.direction);
    }

    setMap(map: GameMap): Camera {
        return new Camera(map, this.position, this.direction);
    }
    ...
    getRays(lineCount: number): Ray[] {
        ...
        for (let i = 0; i < lineCount; i++) {
            ...
            const ray: Ray = new Ray(this.position, rayDirection, () => { });
    ...
    private castRays(lineCount: number): CameraLine[] {
        const cameraLineData: CameraLine[] = Array.from({length: lineCount}, value => CameraLine.getEmpty());
        ...
        for (let i = 0; i < lineCount; i++) {
            ...
            const ray: Ray = new Ray(this.position, rayDirection, (hit) => {
                ...
                // * Math.sin( Vector2.angleBetween( perpendicularDirection, rayDirection ).radians );
                console.log(Math.min(Vector2.angleBetween( perpendicularDirection, rayDirection ).degrees, Vector2.angleBetween( perpendicularDirection.toLength(-1), rayDirection ).degrees));
                cameraLineData[i] = new CameraLine( (1.0 / distanceFromHitToPlane), hit );
            });
            ray.cast(this.viewDistance, this.map);
        }
        ...
        // const rayDirection: Vector2 = currentCameraPlaneLocation.subtract(this.position);
        //     const ray: Ray = new Ray(this.position, rayDirection, (hit) => {
        //         const distanceFromHitToPlane: number = Vector2.distance(currentCameraPlaneLocation, hit.position) * Math.sin( Vector2.angleBetween( perpendicularDirection, rayDirection ).radians );
        //         cameraLineData[0] = ( new CameraLine( (1.0 / distanceFromHitToPlane), hit ) );
        //     });
        //     ray.cast(this.viewDistance, this.map);
